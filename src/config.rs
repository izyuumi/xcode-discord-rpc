...
/// Argument ID for custom config file path
const CONFIG_PATH_ARG_ID: &str = "config";
...
impl AppConfig {
    pub fn new() -> crate::Result<Self> {
        ...
        // Use custom config path if provided, otherwise default location
        if let Some(custom_path) = clap_matches.get_one::<String>(CONFIG_PATH_ARG_ID) {
            let path = PathBuf::from(custom_path);
            log::info!("Using custom config file: {:?}", path);
        ...
        ClapCommand::new("Xcode Discord RPC")
            ...
            .arg(
                Arg::new(CONFIG_PATH_ARG_ID)
                    ...
                    .num_args(1)
                    .value_name("PATH")
                    .help("Path to custom config file (default: ~/.config/xcode-discord-rpc/config.toml)"),
            )
            .arg(
